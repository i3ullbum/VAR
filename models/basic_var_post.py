...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
        ...
        x = self.ln_wo_grad(x + self.drop_path(self.attn(x, attn_bias=attn_bias).mul_(gamma1))).mul(scale1.add(1)).add_(shift1)
        x = self.ln_wo_grad(x + self.drop_path(self.ffn(x).mul(gamma2))).mul(scale2.add(1)).add_(shift2)
        return x
